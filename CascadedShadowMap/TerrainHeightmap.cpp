...
TerrainHeightmap::TerrainHeightmap(UINT quality) : m_quality(std::max((UINT)2, quality)), m_pos({ 0.0f,0.0f }) {}

void TerrainHeightmap::Load() {
...
void TerrainHeightmap::LoadTexture() {
...
void TerrainHeightmap::UpdateTerrain(XMFLOAT4 position) {
    {
        ThrowIfFailed(m_commandAllocator->Reset());
        ...
        m_commandList->Dispatch(m_nVertices >> 9, 1, 1);
...
void TerrainHeightmap::LoadVertices() {
    D3D12_RESOURCE_DESC resourceDesc = {
        D3D12_RESOURCE_DIMENSION_BUFFER,
        0,
